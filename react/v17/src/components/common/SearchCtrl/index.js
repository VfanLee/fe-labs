 ...
 * @param {标题} title
 */
const getTitle = title => <span style={{ fontWeight: 'bold' }}>{title}:</span>;
...
const getComponent = that => {
  ...
  switch (component) {
    case 'input':
      return getInput(restProps);
    case 'inputNum':
      return getInputNum(restProps);
    case 'select':
      return getSelect(restProps);
    case 'period':
      return getPeriod(restProps, that);
    case 'status':
      return getStatus(restProps);
    case 'dates':
      return getDate(restProps);
    case 'btns':
      return getBtnGroup(restProps);
    case 'mutiSelect':
      return getMutiSelect(restProps);
    default:
      console.warn(`不存的组件类型->${component}！！！`);
      return '';
  }
};
...
/**
 * 获取Antd的InputNumber输入框组件
 * @param {标题} title
 * @param {参数} props
 */
const getInputNum = ({ title, props }) => {
  const { style = { width: 80 } } = props;
  return (
    <div className={styles.inputSearch}>
      {getTitle(title)}
      <InputNumber {...props} style={style} />
    </div>
  );
};

/**
 * 获取Antd的DatePicker输入框组件
 * @param {标题} title
 * @param {参数} props
 */
const getDate = ({ title, props }) => {
  const { style = { width: 200 } } = props;
  return (
    <div className={styles.inputSearch}>
      {getTitle(title)}
      <RangePicker style={style} {...props} />
    </div>
  );
};
...
const period = {
  ...
  lastweek: [
    `${moment().add(-1, 'week').day(1).format('YYYY-MM-DD')} 00:00:00`,
    `${moment().add(-1, 'week').day(7).format('YYYY-MM-DD')} 23:59:59`,
  ], // 上周
...
const getPeriod = ({ title, props }, that) => {
  ...
  const getDot = name => (value === name ? <div className={styles.dot} /> : <div className={styles.noDot} />);

  const div = option.map(item => (
    <div
      className={styles.periodItem}
      key={item.value}
      onClick={() => {
        that.setState({ visible: false });
        onClick(item.value, period[item.value] ? period[item.value] : period['today']);
      }}
    >
  ...
  div.push(
    <div key="diy" style={{ display: isDIY ? 'inline-block' : 'none' }}>
      ...
      <span
        style={{ color: value === 'diy' ? '#1890FF' : '#595959', cursor: 'pointer' }}
        onClick={() => that.setState({ visible: !visible })}
      >
        自定义
      </span>
      <RangePicker
        style={{ width: 250, marginLeft: 5, display: visible ? 'inline-block' : 'none' }}
        onChange={(dates, dateStrings) => onClick('diy', [`${dateStrings[0]} 00:00:00`, `${dateStrings[1]} 23:59:59`])}
      />
    </div>
  );
  return (
...
const getStatus = ({ title, props }) => {
  ...
  const div = option.map(item => (
    <div key={item.value} className={styles.periodItem} onClick={() => onClick(item.value)}>
      {showDot ? getDot(item.value) : null}
      <span style={{ color: value === item.value ? '#1890FF' : '#595959' }}>
        {item.name}
        {item.count ? getCount(item.count) : null}
      </span>
    </div>
  ));
  return (
...
const getBtnGroup = ({ option = [] }) => {
  return (
    <div className={styles.btns}>
      {option.map(({ title = '', ...restProps }, index) => (
        <Button key={title + index} {...restProps}>
          {title}
        </Button>
      ))}
    </div>
